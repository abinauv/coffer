...
| Colour: **Paper** (light) and **Slate** (dark), accent **Lapis**. Measured, not asserted — see the header of `tokens.css` | The GitHub social preview image (1280 × 640, uploaded in the repository settings rather than committed)                    |
| Type: **IBM Plex** Sans, Serif and Mono, bundled under SIL OFL 1.1, with the Devanagari cut alongside                     | Installer artwork: the NSIS sidebar image and the macOS disk-image background                                              |
