...
|                  |                                                                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Node             | 22 LTS or newer. `package.json` sets `engines.node: >=22`, and `.nvmrc` says 22 â€” the version CI and every release build use. |
| npm              | Whatever ships with your Node.                                                                                                  |
| A compiler       | No. Nothing in this repo is compiled from C or C++ â€” see Â§3. If something asks you for one, read Â§6 before installing it.   |
...
**Install with `--ignore-scripts`.** A plain `npm ci` â€” or `npm install` on a fresh clone â€”
fails with a page of `node-gyp` output demanding a C++ compiler for a binary that is
already on disk. Â§6 explains why, and why the three lines above are the fix.

If no window appears, read Â§6 before anything else. Two of the traps there produce a
...
[native]   ok   @node-rs/argon2 â€” argon2id hash and verify (m=19456,t=2,p=1)
```
...
the `npm ci` behaviour in Â§6 a trap rather than a requirement â€” the compiler it asks for
would produce a binary the tarball already contains.

Coffer has two native dependencies â€” `better-sqlite3-multiple-ciphers` for the encrypted
...
What can still go wrong is a binary that is simply absent â€” `npm install --omit=optional`,
...
|                        |                                                                                        |
| ---------------------- | -------------------------------------------------------------------------------------- |
| `npm run verify`       | **The gate.** typecheck â†’ lint â†’ format:check â†’ test â†’ test:scripts, in order. |
| `npm run typecheck`    | `typecheck:node` then `typecheck:web` â€” two separate projects.                       |
| `npm run lint`         | ESLint over the repo.                                                                  |
| `npm test`             | Vitest, one pass.                                                                      |
| `npm run test:scripts` | `node --test scripts/mutate.test.mjs` â€” the mutation harness's own tests.            |
| `npm run coverage`     | Vitest with v8 coverage.                                                               |

`npm run verify` runs all five checks. Note that it includes `format:check`, so a file
Prettier would reflow fails the gate â€” run `npm run format` first.
...
`npm run mutate` is the harness that answers "would anything have noticed?" â€” coverage
...
can repeat. The exit code carries the verdict â€” 0 everything killed, 1 something
survived, 2 the run was broken. Every campaign needs a control that changes nothing and a
canary that must die; `CONVENTIONS.md` Â§6 is why, with the ten ways this harness has
...
| `npm run brand:assets`  | Render the README banner, the social preview and the installer artwork. |
...
```
src/main/index.ts      â†’  out/main/index.js       ESM,  Node, externalised deps
src/preload/index.ts   â†’  out/preload/index.js    CJS,  sandboxed, externalised deps
src/renderer/index.htmlâ†’  out/renderer/           ESM,  browser, everything bundled
```
...
`@node-rs/argon2` out of the bundle â€” they are loaded from `node_modules` at runtime,
...
`npm ci` â€” and `npm install` on a fresh clone, or after deleting `node_modules` â€” fails
...
needed (Â§3), and installing one only makes the wrong thing succeed slowly.
...
Electron binary â€” without the second line `node_modules/electron/dist` is empty and
`npm run dev` has nothing to launch. The third is the `postinstall` check run by hand, in
verify-only mode â€” it loads both native modules and does a keyed round-trip, which is the
...
style preference. A sandboxed preload is not a real ES module context â€” Electron loads it
...
`"use strict"; const electron = require("electron");` â€” if it starts with `import`, the
...
spawns, for exactly this reason â€” otherwise its Electron check would quietly downgrade to
...
Compare with the whitespace collapsed, or â€” better, and what the template's own tests do â€”
...
```
src/
â”œâ”€â”€ branding.ts            product name, ids, paths, URLs. The only place they appear.
â”œâ”€â”€ shared/                types + the IPC contract. Imported by all three processes.
â”‚   â”œâ”€â”€ ipc.ts             CofferApi â€” the contract. Twelve groups.
â”‚   â”œâ”€â”€ dto.ts             the shapes that cross IPC
â”‚   â”œâ”€â”€ documents.ts       the five trade-document kinds, as data
â”‚   â”œâ”€â”€ receipts.ts        the four voucher kinds, as data
â”‚   â””â”€â”€ scalars.ts         DecimalString, Timestamp, DateString
â”œâ”€â”€ main/
â”‚   â”œâ”€â”€ index.ts           bootstrap, window, quit
â”‚   â”œâ”€â”€ books/             which database is open, and under which regime
â”‚   â”œâ”€â”€ companies/         registry, create/open/recover/close, backup/restore
â”‚   â”œâ”€â”€ db/                connection, migration runner, migrations/ (0001â€“0022),
â”‚   â”‚                      schema.ts (20 tables), repos/ (one per aggregate)
â”‚   â”œâ”€â”€ domain/            PURE. money/ time/ ledger/ documents/ receipts/
â”‚   â”‚                      inventory/ reports/
â”‚   â”œâ”€â”€ regimes/           types.ts + in-gst/ â€” the internationalisation seam
â”‚   â”œâ”€â”€ security/          argon2, DEK, vault, recovery codes, sealed box
â”‚   â”œâ”€â”€ services/          pdf/ and importers/{csv,xml,zoho,tally}/ â€” built,
â”‚   â”‚                      tested, and not yet reachable over IPC
â”‚   â”œâ”€â”€ ledger/ parties/ items/ units/ documents/ receipts/ numbering/
â”‚   â”‚   company-profile/ regime/    one service per IPC group
â”‚   â””â”€â”€ ipc/               registry, boundary, validators, handlers/
â”œâ”€â”€ preload/
â””â”€â”€ renderer/src/          React: components/, screens/, store/, lib/, styles/
```

`ARCHITECTURE.md` Â§5 is the same tree with the reasoning attached, and it marks the three
things that are still only planned: `main/app/`, `services/excel/` and
`services/mailer/`. Backup is not among them â€” it lives in `companies/`, where the vault
paths are.
...
| ------------------------------------------- | -------------------------------------------------------------------------------- |
| Arithmetic on money, dates or periods       | `src/main/domain/`. No I/O, no `node:*`, no `electron` â€” ESLint enforces this. |
...
Anything touching money, tax or posting uses **golden fixtures** â€” JSON in a
...
The rest â€” what a good PR looks like, what will be rejected â€” is in
