...
nothing installed, so it is worth the thirty seconds. The build-provenance attestation,
below it, says where the file was built.
...
so on its own it cannot say who put either of them there. Every file on a release also has
a build-provenance attestation: a statement, signed during the release workflow's own run,
that a file with exactly this content was built by that workflow from the version's tag.
With the [GitHub CLI](https://cli.github.com/) installed and signed in to any GitHub
account, and your version in place of `0.1.0`:
...
It passes only for a file built by Coffer's release workflow from that tag and not changed
since. A file uploaded to the release page any other way fails it, even with a matching
line in `SHA256SUMS.txt`. **If it reports an error, do not run the installer.**

What it does not do: it is not code signing, so Windows and macOS still warn exactly as
